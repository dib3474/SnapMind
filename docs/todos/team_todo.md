...
- [ ] `#11` Room DB 스키마 설계 (MemoryItem, Tag, N:M 매핑) `DB`
- [ ] `#12` DAO 구현 (insert / update / delete / query) `DB`
- [ ] `#14` Room FTS 검색 인덱스 설정 `DB`
...
- [ ] `#3` 수신 이미지 앱 내부 저장소 복사 및 URI 관리 `CORE`
- [ ] `#13` 태그 저장 및 N:M 매핑 로직 구현 `DB`
...
- [ ] `#18` 카테고리 필터 UI (DrawerLayout 우측 메뉴) `UI`
- [ ] `#21` 핀 고정 / 해제 토글 UI `UI`
- [ ] `#22` 즐겨찾기 탭 — 핀 이미지 갤러리 화면 `UI`
